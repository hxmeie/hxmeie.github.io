...
> **面试系列**：本文是一份完整的模拟面试自问自答稿，可搭配阅读 [《面试如何回答"遇到的最大难题"》](/interview/面试如何回答遇到的最大难题-三个真实案例口述稿/) 和 [《面试反问环节——"你有什么想问我的吗"该怎么答》](/interview/面试反问环节-该问面试官哪些问题/)。
{: .prompt-info }

### 1、自我介绍

面试官您好，我叫何晓猛，本科毕业于天津商业大学计算机科学与技术专业，是一名 Android 高级开发工程师，到现在有十年 Android 开发经验。

这十年我先后在威马汽车、大米未来科技、选课科技、众艺汇智，以及现在的北京百邻，做过出行、在线教育、直播、AI 互动、企业协同办公等多个方向。挑几段有代表性的说：在威马汽车，我做的是即客行出行 App，负责充电优惠券、即时用车等模块，还用 Service + Native 的 Speex 库解决了行程录音上传的文件过大问题；在大米未来科技，我做的是 VIPKID 启蒙 App，从 0 到 1 主导了项目组件化，用 ARouter 完成组件通信，把 App 启动耗时降低了约 50%；在选课科技，我负责考虫直播 App 的多个核心模块，重点治理了大量历史遗留的内存泄漏和 OOM，还把网络层整体改造成了 Kotlin 协程；在众艺汇智，我独立完成了 AI 跳舞学生端和教师端，学生端用 Kotlin + MVVM 做人体姿态识别打分，攻克了姿态对比、透明视频、Unity 与原生交互等难点，线上 Bug 率控制在万分之五以内，教师端则是我从零学 Flutter 独立交付的。

目前我在北京百邻信息技术有限公司，最近这一年主要负责一款叫 Beem 的产品——它是面向海外、主要是中东市场的企业级 IM 加协同办公客户端，形态是「IM 即时通讯 + 音视频会议 + 组织架构 + 轻量办公套件（日历、文档、邮箱、待办、工作台）」。技术栈是 Kotlin + Jetpack Compose + MVI，多仓库 submodule 组织、Gradle 多 flavor 多环境构建。这也是我投入最深、最能代表我能力的一段工作，主要做了这么几件事：

- 一是**音视频播放架构重构**：老方案用本地代理做播放缓存，存在安全隐患，我把它改成了读写分离的设计——播放侧只读缓存、写入职责全部收敛到下载引擎，从架构上根除了多任务并发写缓存的冲突；
- 二是**从 0 到 1 搭了邮箱产品**：负责邮件收发、草稿、附件、富文本编辑这一整套。富文本编辑器是基于 WebView + JsBridge 设计的原生与前端双向通信协议；我还把收件人管理、附件绑定、草稿同步这些复杂交互，按职责拆成了多个独立的协调层，有效控制了主状态机的复杂度；
- 三是**登录账号体系重构**：把登录和「新增账号」两条重复的验证码链路合并，新增了邮箱验证码登录和多身份（手机 / 邮箱 / SSO）验证方式选择；还把倒计时能力下沉成公共组件、用 Room 持久化，解决了设备重启后倒计时状态丢失的问题；
- 另外还做了**海外推送的合规适配**：针对中东市场大量国产厂商设备 GMS 覆盖不足、导致 FCM 推送不可达的问题，推动推送架构按厂商拆分构建变体，并把原来混乱的多厂商配置做了收敛统一。

这一年下来，我最大的收获不只是功能本身，而是沉淀了几条可复用的工程原则，比如「收敛优于推翻」——大改造尽量在调用层逐点迁移，而不是推翻内核重写，把回归风险控制到最小；还有「能力下沉、规则集中」「把依赖网络和不依赖网络的能力解耦」。这些思路让我在做架构决策时更有章法。

技术上，我熟练掌握 Kotlin 和 Java，熟悉 Jetpack Compose、MVI / MVVM 架构和 Coroutine / Flow，也具备用 Flutter 独立开发完整 App 的跨端能力；有大型项目组件化、多仓库分层和多 flavor 构建的经验；在内存泄漏 / OOM 治理、启动优化、播放缓存架构这些性能和稳定性方向积累比较多，对 Handler 原理、View 事件分发、Media3、JsBridge、Unity 交互这些也比较熟。因为做的是海外产品，英文技术文档我也能流畅阅读。平时我保持着写技术博客、持续跟进新技术的习惯，团队协作上也比较主动。

以上就是我的基本情况，谢谢面试官。

> 以上是**完整版**，信息全、适合展开讲；下面是 **60 秒精简口播版**，现场开口用这个更利落。
{: .prompt-tip }

#### 精简口播版（约 60 秒）

面试官您好，我叫何晓猛，Android 高级开发工程师，有十年 Android 开发经验，先后做过出行、在线教育、直播、AI 互动和企业协同办公等方向。

目前在北京百邻，负责一款面向中东市场的企业级 IM + 协同办公客户端 Beem，技术栈是 Kotlin + Jetpack Compose + MVI。这一年我做的几件比较硬核的事：把音视频播放缓存重构成了读写分离架构，根除了并发写缓存的冲突；从 0 到 1 搭了邮箱产品，用 WebView + JsBridge 做富文本编辑器，并把复杂交互按职责拆成多个协调层；还重构了整个登录账号体系。

技术上我精通 Kotlin、Java，熟悉 Compose、MVI、协程和 Flow，也能用 Flutter 独立开发完整 App；在组件化架构、内存泄漏和 OOM 治理、启动和播放性能优化方面经验比较多。以上就是我的基本情况，谢谢面试官。
...
上面的内容偏"文档式"，面试时不适合照着背。下面是我实际口述时的组织方式——**先给框架，再讲流程，然后主动抛出高频追问点，最后一句话收口**。

> 好的，那我先说 Handler 的原理，再说 IdleHandler。
>
> **Handler 这套机制，核心就是四个角色配合：** Handler、Message、MessageQueue 和 Looper。
>
> 我先讲一下它怎么跑起来的。一个线程要用 Handler，得先调 `Looper.prepare()`，这时候会创建一个 Looper，Looper 内部会 new 一个 MessageQueue。然后调 `Looper.loop()` 开启一个死循环，不停地从 MessageQueue 里取消息。主线程这两步是系统在 `ActivityThread.main()` 里帮我们做好的，所以我们平时能直接 new Handler。
>
> 当我们用 Handler 发消息的时候，比如 `sendMessage` 或者 `post`，最终会把消息插到 MessageQueue 里，插入的时候是**按时间排序**的，同时会把这条消息的 `target` 标记成当前这个 Handler。Looper 的循环取到消息以后，就调 `msg.target.dispatchMessage`，回调到发消息那个 Handler 的 `handleMessage`。**关键点是：处理是回到 Handler 所在的线程执行的，所以就实现了线程切换**，子线程发、主线程收，或者反过来。
>
> 这里面试官通常会追问几个点，我一般会主动补一下：
>
> **第一，主线程这个死循环为什么不会卡死、不会 ANR？** 因为队列空的时候，`next()` 会走到 native 层的 `nativePollOnce`，底层是 Linux 的 epoll，线程直接休眠、不占 CPU；有新消息进来的时候再通过管道唤醒。所以它不是空转忙等。ANR 是单条消息处理太久导致的，跟这个循环本身没关系。
>
> **第二，为什么每个线程的 Looper 不会乱？** 因为 Looper 是存在 ThreadLocal 里的，每个线程只能取到自己那一份，保证一个线程一个 Looper。而 Looper 内部又只持有一个 MessageQueue——所以三者是**一个线程 → 一个 Looper → 一个 MessageQueue**的一一对应关系。
>
> **第三，内存泄漏。** 非静态内部类的 Handler 会隐式持有 Activity，如果有延迟消息还在队列里，Activity 就回收不掉。解决办法是用静态内部类加弱引用，或者在 `onDestroy` 里调 `removeCallbacksAndMessages(null)` 把消息清掉。
>
> Handler 大概就是这样。
>
> **接下来说 IdleHandler。**
>
> IdleHandler 是 MessageQueue 提供的一个接口，作用是**在消息队列空闲的时候执行任务**。它只有一个方法 `queueIdle`，返回值是个 boolean：返回 false 就是执行一次后移除，返回 true 就保留，下次空闲还会再触发。
>
> 它的触发时机是在 `MessageQueue.next()` 里面——当**当前没有消息可以马上处理**，也就是队列空了、或者队头消息还没到时间，准备进入 epoll 休眠**之前**，会去遍历执行注册过的 IdleHandler。
>
> 这里我想强调一个容易理解错的点：所谓"空闲"指的是**当前这一刻没活干**，不是说以后都闲下来了。所以它更像是"见缝插针"，利用主线程暂时空闲的间隙来干点低优先级的活。
>
> **实际用途也很明确，主要是启动优化和延迟加载。** 比如把那些不影响首屏的初始化，挪到空闲的时候再做，给启动速度让路；系统自己也用，像 `ActivityThread` 里就有个 `GcIdler`，在空闲时触发 GC；还有像 LeakCanary、Glide 这些库，也会用 IdleHandler 在空闲时做内存检测或者资源清理。
>
> **最后我用一句话总结一下：** Handler 靠 Looper 加 MessageQueue 的消息循环，实现了线程通信和任务调度；而 IdleHandler 就是这套机制里的一个"空闲回调钩子"，让我们能在主线程不忙的时候插入低优先级任务，是做启动优化很常用的一个手段。

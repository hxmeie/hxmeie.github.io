...
Completing 状态属于 job 内部的状态，对于一个外部观察者来说，一个 Completing 态的 job 仍然处于 Active 态，这个 job 在内部正在等待其子 job 执行完。官方注释有个状态流转图，如下所示： ![图 5 协程状态流转](https://cdn.jsdelivr.net/gh/hxmeie/tuchuang@master/images/202305231016466.webp) Job 接口的主要方法有如下几个：
...
delay 函数也是一个挂起函数，它可以非阻塞性的挂起当前线程，并且在设置的时间间隔之后恢复执行，是可被取消的。这里就是挂起 5s 后再执行打印，下图是输出情况，注意看打印的时间： ![图6 非阻塞协程](https://cdn.jsdelivr.net/gh/hxmeie/tuchuang@master/images/202305231019418.webp) 在遇到 delay 后，下面的代码是可以继续执行的，没有被阻塞；当 delay 时间到了，再才会执行第一个打印的代码。如果换成 runBlocking 就不一样了：
...
![图7 阻塞协程](https://cdn.jsdelivr.net/gh/hxmeie/tuchuang@master/images/202305231019680.webp) 执行到 delay 后，会将当前线程阻塞，直到时间到了才能继续往下执行。
...
![图 8 多个 async 并行处理](https://cdn.jsdelivr.net/gh/hxmeie/tuchuang@master/images/202305231020517.webp) 从打印结果可以看出，delay 时间短的 task2 率先完成，总时长为 2s，说明 task1 和 task2 两个任务并行处理了。但是，如果两个 async 方法后面紧接着处理各自的 await 方法，则就是串行处理了，看下面的效果：
...
![图 9 多个 async 串行处理](https://cdn.jsdelivr.net/gh/hxmeie/tuchuang@master/images/202305231020379.webp) 看打印结果，task1 执行完才会执行 task2，总耗时是两个任务耗时的总和。Why? 这是因为 await 函数也是一个挂起函数，协程执行到 await 时会被挂起，当 async 执行完返回结果后，才会继续执行。而在 code 8 中两个 await 函数都是在两个 async 之后，所以在两个 async 中的任务就是并行处理的关系了。

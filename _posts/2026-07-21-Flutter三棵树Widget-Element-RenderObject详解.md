...
本文只讲结构与协作机制；从一帧的完整生命周期（build → layout → paint → 上屏）视角看渲染，见姊妹篇[《Flutter 渲染管线：从 build 到上屏》](/posts/Flutter渲染管线从build到上屏详解/)。
...
- 想进一步看这三棵树在一帧内如何驱动 build/layout/paint/上屏，接着读[《Flutter 渲染管线：从 build 到上屏》](/posts/Flutter渲染管线从build到上屏详解/)。
